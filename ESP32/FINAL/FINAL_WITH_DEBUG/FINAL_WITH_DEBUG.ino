...
void setup() {
    ...
    if (cause == ESP_SLEEP_WAKEUP_EXT0) {
      if (dailyCaptureCount < MAX_CAPTURES_PER_DAY) {
        ...
        if (setupCamera()) {
            ...
            if (!fb) {
                Serial.println("[CAM] Erreur de capture");
                captureOK = false;
            } 
        } else {
            captureOK = false;
        ...
        if (captureOK) {
            ...
            if (WiFi.status() == WL_CONNECTED) {
                if (reconnectMQTT()) {
                    ...
                    Serial.println("[MQTT] Image envoyée");
                } else {
                    captureOK = false;
                }
            } else {
                Serial.println("[WIFI] Échec connexion, envoi annulé");
                captureOK = false;
            }
        }
        ...
        isCooldown = true; 
        Serial.println("[SLEEP] Cooldown 5m");
        esp_sleep_enable_timer_wakeup(EDGE_COOLDOWN_MS * 1000000ULL);
        esp_deep_sleep_start();
        }
        else {
            Serial.println("[CAPTURE] Quota journalier atteint, PIR ignoré");
                    connectToWiFi();
            ...
            if (WiFi.status() == WL_CONNECTED) {
                ...
                if(getLocalTime(&timeinfo)){
                    Serial.printf("[NTP] Heure locale : %02d:%02d:%02d\n",
        ...
        Serial.println("[SLEEP] Dodo : PIR + Timer 24h activés");
  ...
  // --- Réveil TIMER ---
    if (cause == ESP_SLEEP_WAKEUP_TIMER) {
            if (isCooldown) {
                Serial.println("[TIMER] Cooldown terminé. Prêt pour le PIR.");
                isCooldown = false; // Reset pour la prochaine fois
            } else {
                Serial.println("[TIMER] Réveil quotidien : Envoi batterie...");
                connectToWiFi();
                if (WiFi.status() == WL_CONNECTED && reconnectMQTT()) {
                    publishBatteryVoltage();
                    dailyCaptureCount = 0;
                    Serial.println("[SYS] Nouvelle journée : quota reset");
