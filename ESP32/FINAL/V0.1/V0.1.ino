...
void setup() {
    ...
    // Vérifier si un WiFi est déjà enregistré
    preferences.begin("wifi", true);
    String ssid = preferences.getString("ssid", "");
    preferences.end();

    if (ssid == "") {
        // Mode AP pour configuration WiFi
        Serial.println("[CONFIG] Mode AP pour configuration WiFi");
        WiFi.mode(WIFI_AP);
        ...
        // Attendre 3 minutes pour config WiFi/reset
        unsigned long start = millis();
        while (millis() - start < WIFI_CONFIG_TIME_MS) {
            server.handleClient();
        }

        // Après 3 minutes, désactiver AP
