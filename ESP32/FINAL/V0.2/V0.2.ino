...
#define WIFI_CONFIG_TIME_MS 180000  // 3 minutes
...
void setup() {
  ...
  if (ssid == "") {
      // Aucun WiFi configuré → AP actif pour config
      Serial.println("[CONFIG] Aucun WiFi configuré, mode AP actif");
      WiFi.mode(WIFI_AP);
      WiFi.softAP(ap_ssid, ap_password);
      Serial.print("[AP] IP locale AP : "); Serial.println(WiFi.softAPIP());
      ...
      while (millis() - start < WIFI_CONFIG_TIME_MS) {
          server.handleClient(); // Serveur web pour config
      }

      WiFi.softAPdisconnect(true);
      apActive = false;
      Serial.println("[AP] Temps de configuration écoulé, AP désactivé");
  } else {
      // WiFi déjà configuré → connexion immédiate
      Serial.print("[WIFI] Tentative de connexion à "); Serial.println(ssid);
      WiFi.mode(WIFI_STA);
      ...
      if (WiFi.status() == WL_CONNECTED) {
          ...
          startWebServer();
          Serial.print("start webserser");          
      } else {
          Serial.println("\n[WIFI] Échec connexion, AP non activé");
      }

      // Attendre 3 minutes même si le WiFi est configuré
      Serial.println("[INFO] Attente 3 minutes avant fonctionnement normal pour reset ou debug");
      unsigned long start = millis();
      while (millis() - start < WIFI_CONFIG_TIME_MS) {
          server.handleClient();
          delay(100);
    ...
    if (cause == ESP_SLEEP_WAKEUP_EXT0) {
        Serial.println("[CAPTURE] Déclenchement");
        dailyCaptureCount++;
    ...
    // Sinon attendre front montant sur trigger
...
// ===================================================
// … Ici tu peux garder toutes tes fonctions existantes 
// htmlConfigPage(), htmlResetPage(), handleSave(), handleDoReset(), connectToWiFi(), setupCamera(), publishImage(), etc.
// ===================================================
...
bool setupWiFi() {
    ...
    while (WiFi.status() != WL_CONNECTED) {
        ...
        if (millis() - start > 20000) {
            Serial.println("[WIFI][ERREUR] Timeout");
            return false;
        }
    }
